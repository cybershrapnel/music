...
if (isset($_GET['song'])) {
    ...
    $baseDirectory = $_SERVER['DOCUMENT_ROOT'] . ($isVideo ? "/video/videos/" : "/music/mp3s/"); // Choose directory based on file type
...
echo $filePath;
echo str_replace($_SERVER['DOCUMENT_ROOT'], "https://" . $_SERVER['HTTP_HOST'], $filePath);

        return file_exists($filePath) ? str_replace($_SERVER['DOCUMENT_ROOT'], "https://" . $_SERVER['HTTP_HOST'], $filePath) : false;
    }
...
<div style='text-align: center;max-width: 75%;justify-self: center;'>";
...
<video controls autoplay>
                <source src='" . htmlspecialchars($mp3Url) . "' type='video/mp4'>
...
if (spinnerLink) {
    const newHref = `https://suno.com/song/${song.uuid}`;
    spinnerLink.href = newHref;
    console.log(`Updated spinner link to: ${newHref}`);
} else {
